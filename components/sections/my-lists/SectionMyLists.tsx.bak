...
const SectionMyLists = () => {
	...
	const onDeleteList = async ()=>{
		try {
			...
			let response: any;
			response = await API.graphql(graphqlOperation(deleteList, {
				input: {
					id:deleteListId,
				},
			  }));
        ...
        {isTrailAccount ? (
			...
			{groupname ? (
				<QueryLoader
					...
					options={{ groupId: groupname, sortDirection: 'DESC' }}
					limit={999}
					isLoadAll
				/>
			) : null}
			<Modal isActive={isModalActive} onCancel={onCancel}>
				<MyListForm onCreate={onCreate} onUpdate={onUpdate} onCancel={onCancel} itemsList={listItems} />
			</Modal>
			<ExportContacts
