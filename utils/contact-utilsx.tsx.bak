...
import { Contact, ContactCompany, ContactPerson, RampedUpFilter, SelectOption } from '../types';
import { arrayToCsv, download } from './helper-utils';

export const getExportLabels = () => keysToExport.map((key) => sentenceCase(key));

export const getExportData = (data: Contact) => {
	const newItem: any = {};
...
export const searchContactItems = (items: Contact[], query: string) => {
	const fuse = new Fuse(items, contactFuseOptions);
	return fuse.search(`"${query}"`).map((item) => item.item);
};

const applyEmployeesCountFilter = (contact: Contact, countId: number) => {
...
export const applyContactFilters = (contacts: Contact[], filter: RampedUpFilter) => {
	let fuse;
	...
	if (filter?.companyRevenues?.length) {
		...
		results = fuse
			.search(filter.companyRevenues.map((item) => `"${item.label}"`).join('|'))
			.map((item) => item.item);
	}
	...
	if (filter?.seniorityLevels?.length) {
		...
		results = fuse
			.search(filter.seniorityLevels.map((item) => `"${item.value}"`).join('|'))
			.map((item) => item.item);
	}
	...
	if (filter.urlsToSuppress?.length) {
		fuse = new Fuse(results, { ...contactFuseOptions, keys: ['companyDomain'] });
		results = fuse
			.search(filter.urlsToSuppress.map((item) => `!"${item.value}"`).join(' '))
			.map((item) => item.item);
	}
	...
	if (filter.titlesToSuppress?.length) {
		...
		results = fuse
			.search(filter.titlesToSuppress.map((item) => `!"${item.value}"`).join(' '))
			.map((item) => item.item);
	}
	...
	if (filter.emailsToSuppress?.length) {
		...
		results = fuse
			.search(filter.emailsToSuppress.map((item) => `!"${item.value}"`).join(' '))
			.map((item) => item.item);
	}
	...
	if (filter.hasLocalAddress) {
		results = contacts.filter(
			(item) => item.contactOfficeAddress || item.companyAddressLine1 || item.companyAddressLine2
		);
	}
...
export const getEmailValidityStatus = (contact: Contact) => {
	let value;
